(function () {
  ...
  const SOCIALS = Object.freeze([
    ...
    { key: 'instagram', label: 'INS', hrefKey: 'instagram', fallbackHref: 'https://www.instagram.com/yourusername/' },
  ...
  function ensureFallbackTemplate(row) {
    ...
    template.dataset.socialIconFallback = 'twitter-email-originals';
    Array.from(row.children).forEach((wrapper) => {
      if (/^(twitter|email)$/i.test(text(wrapper))) template.content.appendChild(wrapper.cloneNode(true));
    });
