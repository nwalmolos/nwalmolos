(function () {
  ...
  function findSocialRow(section) {
    const appliedRow = section.querySelector('[data-local-social-icons]');
    if (appliedRow) return appliedRow;
    const anchors = Array.from(section.querySelectorAll('a'));
    const github = anchors.find((anchor) => /^github$/i.test(text(anchor)));
    if (github && github.parentElement && github.parentElement.parentElement) {
      return github.parentElement.parentElement;
    }
    return Array.from(section.querySelectorAll('div')).find((node) => {
      const labels = Array.from(node.children).map((child) => text(child));
      return labels.some((label) => /^github$/i.test(label)) && labels.length >= 2;
    }) || null;
  }

  function ensureFallbackTemplate(row) {
    let template = row.querySelector(':scope > template[data-social-icon-fallback]');
    if (template) return template;
    template = document.createElement('template');
    template.dataset.socialIconFallback = 'github-twitter-email-originals';
    Array.from(row.children).forEach((wrapper) => {
      if (/^(github|twitter|email)$/i.test(text(wrapper))) template.content.appendChild(wrapper.cloneNode(true));
    });
  ...
  function makeLabel(label) {
    ...
    span.className = 'hidden sm:inline font-mono tracking-wider text-xs uppercase';
    span.textContent = label;
    return span;
  }

  function apply(contactContent) {
    const section = document.querySelector('#contact');
    if (!section) return false;
    const row = findSocialRow(section);
    if (!row) return false;

    const fallbackTemplate = ensureFallbackTemplate(row);
    const currentWrappers = Array.from(row.children).filter((node) => node.tagName !== 'TEMPLATE');
    const baseWrappers = currentWrappers.length >= 3
      ? currentWrappers.slice(0, 3)
  ...
  window.NM_SOCIAL_ICONS = Object.freeze({
    version: '16.24.1',
